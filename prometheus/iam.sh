...
OIDC_PROVIDER=$($AUTH exec $AWS_ACCOUNT -- aws eks describe-cluster --name $CLUSTER --query "cluster.identity.oidc.issuer" --output text | sed -e "s/^https:\/\///")
...
#
OLD_TRUST_RELATIONSHIP=$($AUTH exec $AWS_ACCOUNT -- aws iam get-role --role-name $SERVICE_ACCOUNT_IAM_ROLE --query 'Role.AssumeRolePolicyDocument.Statement[]' --output json)
...
#
SERVICE_ACCOUNT_IAM_POLICY_ID=$($AUTH exec $AWS_ACCOUNT -- aws iam get-policy --policy-arn $SERVICE_ACCOUNT_IAM_POLICY_ARN --query 'Policy.PolicyId' --output text)
if [ "$SERVICE_ACCOUNT_IAM_POLICY_ID" = "" ]; 
then
  echo "Creating a new permission policy $SERVICE_ACCOUNT_IAM_POLICY"
  $AUTH exec $AWS_ACCOUNT -- aws iam create-policy --policy-name $SERVICE_ACCOUNT_IAM_POLICY --policy-document file://PermissionPolicy.json 
else
...
then
  ...
  #
  SERVICE_ACCOUNT_IAM_ROLE_ARN=$($AUTH exec $AWS_ACCOUNT -- aws iam create-role \
  ...
  #
  $AUTH exec $AWS_ACCOUNT -- aws iam attach-role-policy --role-name $SERVICE_ACCOUNT_IAM_ROLE --policy-arn $SERVICE_ACCOUNT_IAM_POLICY_ARN  
else
  ...
  #
  $AUTH exec $AWS_ACCOUNT -- aws iam update-assume-role-policy --role-name $SERVICE_ACCOUNT_IAM_ROLE --policy-document file://TrustPolicy.json
fi
...
#
$AUTH exec $AWS_ACCOUNT -- eksctl utils associate-iam-oidc-provider --cluster $CLUSTER --approve
